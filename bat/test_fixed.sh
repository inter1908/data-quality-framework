#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# TEST FIXED - Data Quality Framework
# ═══════════════════════════════════════════════════════════════════════════════
# 👤 AUTORE: Alberto Robetti
# 📅 DATA: 12 Novembre 2025
# 🎯 SCOPO: Esegue test completo validazione file FIXED (fixed-width) demo
# ═══════════════════════════════════════════════════════════════════════════════
...
# Conta file generati
count_ok=$(ls -1 demo/fixed/output/success/*.ok 2>/dev/null | wc -l)
count_ko=$(ls -1 demo/fixed/output/error/*.ko 2>/dev/null | wc -l)
count_codici=$(ls -1 demo/fixed/output/report/*.codici 2>/dev/null | wc -l)
count_stats=$(ls -1 demo/fixed/output/statistics/*.stats 2>/dev/null | wc -l)

echo "  ✅ File validati (.ok):           $count_ok"
[ $count_ok -gt 0 ] && ls -1 demo/fixed/output/success/*.ok 2>/dev/null | xargs -n1 basename
echo ""

echo "  ❌ File scartati (.ko):           $count_ko"
[ $count_ko -gt 0 ] && ls -1 demo/fixed/output/error/*.ko 2>/dev/null | xargs -n1 basename
echo ""

echo "  📋 Anagrafica errori (.codici):   $count_codici"
[ $count_codici -gt 0 ] && ls -1 demo/fixed/output/report/*.codici 2>/dev/null | xargs -n1 basename
echo ""

echo "  📊 Report statistiche (.stats):   $count_stats"
[ $count_stats -gt 0 ] && ls -1 demo/fixed/output/statistics/*.stats 2>/dev/null | xargs -n1 basename
echo ""
...
case $choice in
    1)
        KO_FILE=$(ls -1t demo/fixed/output/error/*.ko 2>/dev/null | head -1)
        [ -n "$KO_FILE" ] && cat "$KO_FILE"
        ;;
    2)
        CODICI_FILE=$(ls -1t demo/fixed/output/report/*.codici 2>/dev/null | head -1)
        [ -n "$CODICI_FILE" ] && cat "$CODICI_FILE"
        ;;
    3)
        STATS_FILE=$(ls -1t demo/fixed/output/statistics/*.stats 2>/dev/null | head -1)
