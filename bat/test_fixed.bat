@echo off
REM ═══════════════════════════════════════════════════════════════════════════════
REM TEST FIXED - Data Quality Framework
REM ═══════════════════════════════════════════════════════════════════════════════
REM 👤 AUTORE: Alberto Robetti
REM 📅 DATA: 12 Novembre 2025
REM 🎯 SCOPO: Esegue test completo validazione file FIXED (fixed-width) demo
REM ═══════════════════════════════════════════════════════════════════════════════

echo.
echo ═══════════════════════════════════════════════════════════════════════════════
echo   DATA QUALITY FRAMEWORK v1.0.0 - TEST FIXED
echo ═══════════════════════════════════════════════════════════════════════════════
echo.
...
REM Esegui test FIXED
echo ═══════════════════════════════════════════════════════════════════════════════
echo 🚀 ESECUZIONE TEST FIXED - Flusso DIPENDENTI_FIXED
echo ═══════════════════════════════════════════════════════════════════════════════
echo.
...
for %%f in ("demo\fixed\output\success\*.ok") do set /a count_ok+=1
for %%f in ("demo\fixed\output\error\*.ko") do set /a count_ko+=1
for %%f in ("demo\fixed\output\report\*.codici") do set /a count_codici+=1
for %%f in ("demo\fixed\output\statistics\*.stats") do set /a count_stats+=1

echo   ✅ File validati (.ok):           %count_ok%
if %count_ok% GTR 0 dir /B "demo\fixed\output\success\*.ok"
echo.

echo   ❌ File scartati (.ko):           %count_ko%
if %count_ko% GTR 0 dir /B "demo\fixed\output\error\*.ko"
echo.

echo   📋 Anagrafica errori (.codici):   %count_codici%
if %count_codici% GTR 0 dir /B "demo\fixed\output\report\*.codici"
echo.

echo   📊 Report statistiche (.stats):   %count_stats%
if %count_stats% GTR 0 dir /B "demo\fixed\output\statistics\*.stats"
echo.
...
if errorlevel 3 (
    for /f "delims=" %%f in ('dir /b /o-d "demo\fixed\output\statistics\*.stats" 2^>nul') do (
        type "demo\fixed\output\statistics\%%f"
        echo.
        pause
        goto :show_actions
    )
...
if errorlevel 2 (
    for /f "delims=" %%f in ('dir /b /o-d "demo\fixed\output\report\*.codici" 2^>nul') do (
        type "demo\fixed\output\report\%%f"
        echo.
        pause
        goto :show_actions
    )
...
if errorlevel 1 (
    for /f "delims=" %%f in ('dir /b /o-d "demo\fixed\output\error\*.ko" 2^>nul') do (
        type "demo\fixed\output\error\%%f"
        echo.
        pause
        goto :show_actions
    )
...
echo.
echo ═══════════════════════════════════════════════════════════════════════════════
echo   Test FIXED completato. Grazie per aver usato Data Quality Framework!
echo ═══════════════════════════════════════════════════════════════════════════════
echo.
pause
